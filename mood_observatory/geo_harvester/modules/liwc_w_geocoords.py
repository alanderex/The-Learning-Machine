...
def set_up_dataframe(csv_file, category_names):
    ...
    df = DataFrame(
        pd.read_csv(csv_file, encoding="utf-8", engine="python", error_bad_lines=False)
    )
    try:
        df["created_at"] = df["created_at"].apply(
            lambda d: datetime.strptime(d, "%a %b %d %H:%M:%S %z %Y").strftime(
                "%H:%M_%d-%m-%Y"
            )
        )
    except TypeError:
...
def count_and_insert(df, parse_fn):
    ...
    for tweet in df["text"]:
        ...
        text_counts = Counter(
            category for token in text_tokens for category in parse_fn(token)
        )
        for (
            count_category
        ) in text_counts:  # insert the LIWC values as proportion of word_count
            df.at[index, count_category] = text_counts[count_category] / (
                df.iloc[index]["word_count"]
            )
        index += 1
